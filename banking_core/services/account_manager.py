...
class AccountManager:
    ...
    def transfer(self, source_card, target_card, amount):
        try:
            ...
            target_exists = self.db.fetch_one("SELECT * FROM card WHERE number = %s", (target_card,))
            ...
            self.db.execute_query('UPDATE card SET balance = balance - %s WHERE number = %s', (amount, source_card))
            self.db.execute_query('UPDATE card SET balance = balance + %s WHERE number = %s', (amount, target_card))
    ...
    def close_account(self, card_number):
        ...
        self.db.execute_query("DELETE FROM card WHERE number = %s", (card_number,))
    ...
    def set_daily_limit(self, card_number):
        ...
        if PinHasher.check_pin(stored_pin, pin):
            try:
    ...
    def lock_account_after_failed_attempt(self, card_number):
        ...
        if failed_attempts >= 3:
            self.db.execute_query("UPDATE card SET locked = 1 WHERE number = %s", (card_number,))
