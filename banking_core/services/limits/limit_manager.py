...
class LimitManager:
    ...
    def set_daily_limit(self, card_number):
        try:
            ...
            stored_pin = self._get_stored_pin(card_number)

            if not PinHasher.check_pin(stored_pin, pin):
                print("Invalid PIN.")
                return

            new_limit = self._get_new_limit()
            self._update_or_add_limit(card_number, new_limit, limit_data)
            print(f"Your daily limit has been set to {new_limit}.")

        except Exception as e:
            print(f"An error occurred while setting the daily limit: {e}")

    def _get_stored_pin(self, card_number):
    ...
    def _update_or_add_limit(self, card_number, new_limit, limit_data):
